...
int main(int argc, char **argv)
{
	...
	while(gets(option))
	{
		...
		else if(!strncmp(option,"multi",5))
		{
			char *token,*str1,*str2,*saveptr1;
			char temp[9][ALIASLEN];
			int num;
			str1=option;
			token=strtok_r(str1," ",&saveptr1);            // printf("1--->%s",token);
			str1=NULL;
			token=strtok_r(str1," ",&saveptr1);             //printf("2--->%s",token);
			num=*token-48;  //printf("NUM--->%d",num);
			//ptr=strtok(0," ");
			str1=NULL;
			for(i=0;i<num;i++)
			{
				...
				//printf("--->%s",token);
				str2=token;
				memset(temp[i],0,sizeof(char)*ALIASLEN);
				if(str2!=NULL)
				{
					aliaslen=strlen(str2);
					if(aliaslen > ALIASLEN)
						str2[ALIASLEN]=0;
					strcpy(temp[i],str2);
					while(*str2)
